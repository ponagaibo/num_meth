...
public class FractionalSteps extends Parabolic2DMethods {
    int k;
    double tau;
    double hx;
    double hy;
...
//        System.out.println("Max error = " + maxError + " at " + errI + " : " + errJ);
        ...
        while (time < k) {
            ...
            for (int j = 1; j <= valueN2 - 1; j++) {
                ...
                d_terms_x[0] = -prevSolution[1][j]
                        - tau / 2.0 * f.apply(hx * 1, hy * j, tau * prevTime)
                        - sigma_x * phi3.apply(hy * j, tau * time);
                for (int i = 2; i < dim_x; i++) {
                    d_terms_x[i - 1] = -prevSolution[i][j]
                            - tau / 2.0 * f.apply(hx * i, hy * j, tau * prevTime);
                }
                d_terms_x[dim_x - 1] = -prevSolution[valueN1 - 1][j]
                        - tau / 2.0 * f.apply(hx * (valueN1 - 1), hy * j, tau * prevTime)
