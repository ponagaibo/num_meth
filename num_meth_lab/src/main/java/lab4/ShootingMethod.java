...
public class ShootingMethod {
    ...
    void shootingMethod() {
        double old_slope = 2.;
        RungeKuttaMethod em = new RungeKuttaMethod(h, a, b, a, old_slope, y_a, f1, f2, real_f);
        double old_check = em.modifiedRungeKuttaMethod(h, a, old_slope, y_a);

        double slope = 2.2;
        double check = em.modifiedRungeKuttaMethod(h, a, slope, y_a);
        double eps = 0.0001;
        //System.out.println("s: " + slope + ", old_s: " + old_slope + ", c: " + check + ", old_c: " + old_check);
        int cnt = 0;
        while (Math.abs(check - y_b) >= eps && Math.abs(slope - old_slope) >= eps) {
            double new_slope = slope - (check - y_b) * (slope - old_slope) / (check - old_check);
            double new_check = em.modifiedRungeKuttaMethod(h, a, new_slope, y_a);
            old_slope = slope;
            old_check = check;
            slope = new_slope;
            check = new_check;
...
//        System.out.println("slope: " + slope + ", check: " + old_check + ", cnt: " + cnt);
        em.rungeKuttaMethodWithPrint(h, a, slope, y_a);
    }
}
